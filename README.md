...
A super small Alpine image with OptiPNG installed. The project icon is from [cre.ativo mustard, HK from the Noun Project](docs/icon/icon.json)
...
```bash
docker inspect -f '{{ index .Config.Labels "org.label-schema.<LABEL>" }}' jrbeverly/optipng
```
...
```bash
docker inspect -f '{{ index .Config.Labels "org.doc-schema.<LABEL>" }}' jrbeverly/optipng
```
...
```bash
docker inspect -f '{{ index .Config.Labels "org.doc-schema.user" }}' jrbeverly/optipng:baseimage
docker inspect -f '{{ index .Config.Labels "org.doc-schema.group" }}' jrbeverly/optipng:baseimage
```
